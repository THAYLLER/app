...
class WelcomeScreen extends Component {
  ...
  handleNextClick = index => this.setState({ labelColor: this.state.onboarding[index].nextLabelColor, activeBGcolor: this.state.onboarding[index].backgroundColor, ind: index })
    ...
    return (
      <>
        ...
        <AppIntro
          ...
          // onSlideChange={(index) => }
          onDoneBtnClick={() => navigation.goBack()}
        >
          {onboarding.map((item, index) => (
            <View style={{ flex: 1, backgroundColor: `${item.backgroundColor}` }} key={() => index.toString()}>
              <View level={item.level} style={{ marginTop: height > 800 ? 160 : 80, marginBottom: height > 800 ? height - width - 460 : height - width - 350 }}>
